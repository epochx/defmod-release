...
def main():
    ...
    _, model_name = args.model_name_or_path.split("/")
    ...
    # in the environment
    accelerator = (
        Accelerator(log_with=args.report_to, project_dir=output_dir)
        ...
        if args.push_to_hub:
            if args.hub_model_id is None:
                repo_name = get_full_repo_name(
                    Path(output_dir).name, token=args.hub_token
                )
            else:
                repo_name = args.hub_model_id
            create_repo(repo_name, exist_ok=True, token=args.hub_token)
            repo = Repository(
                output_dir, clone_from=repo_name, token=args.hub_token
            )
    ...
    for dataset in args.datasets:
        language, dict_name = os.path.basename(os.path.normpath(dataset)).split(
            "-"
        )
        train_file = os.path.join(dataset, "train.json")
        validation_file = os.path.join(dataset, "val.json")
        data_files = {}
    ...
    train_datasets = []
    valid_datasets = []
    for raw_dataset in raw_datasets_list:
        train_dataset = raw_dataset["train"]

        train_preprocess_fn = get_preprocess_function(
            tokenizer=tokenizer,
            args=args,
            language=raw_dataset["language"],
        ...
        with accelerator.main_process_first():
            train_dataset = train_dataset.map(
                ...
                desc=f"Running tokenizer on train of {raw_dataset['dict_name']}",
            )
        ...
        valid_preprocess_fn = get_preprocess_function(
            tokenizer=tokenizer,
            args=args,
            language=raw_dataset["language"],
        ...
        with accelerator.main_process_first():
            valid_dataset = valid_dataset.map(
                ...
                load_from_cache_file=not args.overwrite_cache,
                desc=f"Running tokenizer on valid of {raw_dataset['dict_name']}",
            )
    ...
    if "mt5" in args.model_name_or_path:
        # just mix cross-language examples inside batches in the case of mt5
        train_sizes = [len(train_dataset)]
    else:
        # sample single-language batches for mbarts
        train_sizes = train_dataset.cumulative_sizes

    train_batch_sampler = MultiLingualBatchSampler(
        train_sizes,
        batch_size=args.per_device_train_batch_size,
        drop_last=False,
    )

    eval_dataset = ConcatDataset(valid_datasets)
    ...
    if "mt5" in args.model_name_or_path:
        ...
        # sample single-language batches for mbarts
        eval_sizes = eval_dataset.cumulative_sizes
    ...
    output_output_dir = os.path.join(output_dir, "outputs")
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            language_batch = batch.pop("language")
            if "mbart" in args.model_name_or_path:
                assert torch.all(language_batch[0] == language_batch)
            outputs = model(**batch)
            loss = outputs.loss
            ...
            with torch.no_grad():
                ...
                try:
                    ...
                    if random.random() * 10 >= 9:
                        progress_bar.write(f"Input  : {decoded_inputs[0]}")
                        progress_bar.write(f"Gen    : {decoded_preds[0]}")
                        progress_bar.write(f"Target : {decoded_labels[0]}")
        ...
        with open(
            os.path.join(output_output_dir, "current_output.json"), "w"
        ) as f:
        ...
        # Save best model
        if metric_values["comet"]["mean_score"] > best_score:
            print("NEW BEST MODEL")
            best_score = metric_values["comet"]["mean_score"]
            best_epoch = epoch
            best_output = current_output
            best_output_dir = os.path.join(output_output_dir, "best_model")
            accelerator.save_state(best_output_dir)
            with open(
                os.path.join(output_output_dir, "best_epoch.txt"), "w+"
            ) as f:
                f.write(str(best_epoch))

            with open(
                os.path.join(output_output_dir, "best_output.json"), "w"
            ) as f:
    ...
    if output_dir is not None:
        ...
        with open(os.path.join(output_dir, "all_results.json"), "w") as f:
            json.dump({"metrics": metric_values}, f)
